...
//Действительно получается очень костыльное решение где map по сути выступает как хитрый if...
...
function t6() {
	...
	document.querySelector('.out-6').innerHTML = `b6_res = ${b6_res}`;
}
...
function t7() {
	...
	b7_res = b7.filter(item => (typeof (item) === 'string'));

	document.querySelector('.out-7').innerHTML = `b7_res = [${b7_res}]`;
...
/*
Без return это опять же хитрый if, а так как return true в callback выдает item то ловить индексы таким способом 
невозможно, только внутри функции через дополнительный массив, а это проще делать через обычный цикл и сравнение
*/
...
function t8() {
	...
	document.querySelector('.out-8').innerHTML = `b8_res = [${b8_res}]`;
...
/*
Буду очень благодарен если в качестве обратной связи вы скажете насколько корректно пользоваться filter, 
но не напрямую с возвратом callback, а как тут частично как результат filter, а частично как хитрое условие
*/
...
function t9() {
	...
	document.querySelector('.out-9').innerHTML = `b9_num = [${b9_num}] <br>`;
	document.querySelector('.out-9').innerHTML += `b9_string = [${b9_string}]`;
...
function t10() {
	...
	/*
	Вот на этом моменте ОГРОМНОЕ СПАСИБО ЗА КУРС!!!
	*/
	b10_res = b10.filter(item => item.find(item2 => (item2 === 3)));
...
/*
Как быстро кончились задания в этом уроке (((
*/